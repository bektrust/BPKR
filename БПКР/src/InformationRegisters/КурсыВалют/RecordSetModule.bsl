...
Процедура ПриЗаписи(Отказ, Замещение)
	...
	Для Каждого ЗаписьОсновнойВалюты Из ЭтотОбъект Цикл
		Если ЗаписьОсновнойВалюты.Валюта = ВалютаРегламентированногоУчета
			И НЕ ЗаписьОсновнойВалюты.Курс = 1 Тогда 
			ТекстСообщения = НСтр("ru = 'Установка и изменение курсов валют для Валюты регламентированного учета не предусмотрена.'");
			ОбщегоНазначения.СообщитьПользователю(ТекстСообщения,, "Валюта",, Отказ);
			Возврат;
		КонецЕсли;	
	КонецЦикла;	
	// Конец БПКР
