...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("Операции") Тогда
		...
		Если Объект.ВидОперации = Перечисления.ВидыОперацийППВ.ПрочийПриход Тогда
...
// Процедура - обработчик события ПриИзменении поля ввода Вид оплаты.
//
&НаКлиенте
Процедура ВидОплатыПриИзменении(Элемент)
	РассчитатьСуммуУслуг();
	Объект.СчетРасходов = СчетРасходовПоВидуОплаты(Объект.ВидОплаты);
КонецПроцедуры
...
Процедура РасшифровкаПлатежаСуммаПлатежаПриИзменении(Элемент)
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППВ.ПоступлениОтПродажПоДоговорамЭквайринга") Тогда 
		РассчитатьСуммуУслуг();
	КонецЕсли;	
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	Если Объект.ВидОперации = Перечисления.ВидыОперацийППВ.ОплатаОтПокупателя
		...
		Элементы.СчетРасходов.Видимость = Ложь;
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийППВ.ВозвратОтПодотчетника Тогда 
		Элементы.Контрагент.Видимость = Ложь;
		Элементы.ДоговорКонтрагента.Видимость = Ложь;
		Элементы.СчетРасчетов.Видимость = Ложь;
	...
	ИначеЕсли Объект.ВидОперации = Перечисления.ВидыОперацийППВ.ПоступлениОтПродажПоДоговорамЭквайринга Тогда 
		...
		Элементы.СчетРасходов.Видимость = Истина;
...
Процедура ЗаполнитьВидыОпераций()
	...
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ОплатаОтПокупателя);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ВозвратОтПоставщика);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ВозвратОтСотрудника);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ВозвратОтПодотчетника);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.РасчетыПоЗаймам);
	Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ПрочийПриход);
	Если БухгалтерскийУчетВызовСервераПовтИсп.ВедетсяУчетРозничныхПродаж() Тогда 
		Элементы.ВидОперации.СписокВыбора.Добавить(Перечисления.ВидыОперацийППВ.ПоступлениОтПродажПоДоговорамЭквайринга);
	КонецЕсли;
...
// Функция - Счет расходов по виду оплаты
//
// Параметры:
//  ВидОплаты	 - СправочникСсылка.ВидОплаты	 - Вид оплаты, по которому нужно получить счет расходов
// 
// Возвращаемое значение:
//  СчетРасходов - Счет расходов по отражению комиссии банка
//
&НаСервереБезКонтекста
Функция СчетРасходовПоВидуОплаты(ВидОплаты)
	Возврат ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ВидОплаты, "СчетРасходов");
КонецФункции // СчетРасходовПоВидуОплаты()

&НаСервере
Процедура РассчитатьСуммуУслуг()

	СуммаПлатежа = Объект.РасшифровкаПлатежа.Итог("СуммаПлатежа");
	ПроцентБанковскойКомиссии = Справочники.ВидыОплатОрганизаций.ЭквайрингПроцентУслугБанкаПоВидуОплаты(Объект.ВидОплаты, СуммаПлатежа);	
	Объект.СуммаУслуг = СуммаПлатежа * ПроцентБанковскойКомиссии / 100; 
...
Процедура ОбработатьИзменениеБанковскогоСчетаОрганизации()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППВ.ВозвратОтСотрудника")
		Или Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийППВ.ВозвратОтПодотчетника")
