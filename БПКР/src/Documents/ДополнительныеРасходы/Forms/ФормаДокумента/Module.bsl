...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли (ИмяСобытия = "ПодборПоДокументамПроизведен_Заполнить" Или ИмяСобытия = "ПодборПоДокументамПроизведен_Добавить"
		ИЛИ ИмяСобытия = "ПодборПоДокументамПроизведен_Заполнить_ОС" Или ИмяСобытия = "ПодборПоДокументамПроизведен_Добавить_ОС")  
		...
		Если ЗначениеЗаполнено(АдресДокументовВХранилище) Тогда
			...
			ИначеЕсли ИмяСобытия = "ПодборПоДокументамПроизведен_Заполнить_ОС" Тогда 
				ЗаполнитьПоДокументамОснованиямОС(МассивДокументов);	
			ИначеЕсли ИмяСобытия = "ПодборПоДокументамПроизведен_Добавить" Тогда
				ДобавитьСтрокиТабличнойЧастиИзДокументаПоступления(МассивДокументов);
			Иначе
				ДобавитьСтрокиТабличнойЧастиИзДокументаПоступленияОС(МассивДокументов);	
			КонецЕсли;
		КонецЕсли;
...
Процедура ЗаполнитьОСПоПоступлению(Команда)
	...
	ПараметрыПодбора.Вставить("ДатаОтбора", ДатаДокумента);
	ПараметрыПодбора.Вставить("УникальныйИдентификаторФормыВладельца", УникальныйИдентификатор);
	ПараметрыПодбора.Вставить("СпособЗаполнения", "Заполнить_ОС");
	ОткрытьФорму("Документ.ДополнительныеРасходы.Форма.ФормаПодбораПоДокументам", ПараметрыПодбора);
КонецПроцедуры

&НаКлиенте
Процедура ДобавитьОСИзПоступления(Команда)
	ПараметрыПодбора = Новый Структура;
	ПараметрыПодбора.Вставить("ДатаОтбора", ДатаДокумента);
	ПараметрыПодбора.Вставить("УникальныйИдентификаторФормыВладельца", УникальныйИдентификатор);
	ПараметрыПодбора.Вставить("СпособЗаполнения", "Добавить_ОС");
	ОткрытьФорму("Документ.ДополнительныеРасходы.Форма.ФормаПодбораПоДокументам", ПараметрыПодбора);
КонецПроцедуры
...
Процедура УстановитьВидимостьДоступностьЭлементов()
	...
	Если ПризнакСтраны = Перечисления.ПризнакиСтраны.ЕАЭС Тогда
		Элементы.БезналичныйРасчет.Видимость = ДанныеУчетнойПолитики.ПлательщикНСП И Объект.ПоступлениеОтНерезидента;
		
		Элементы.ЭтоПоступлениеПоЧеку.Видимость = Ложь;

		Элементы.ЗначениеСтавкиНДС.Видимость = Истина;
		Элементы.ЗначениеСтавкиНСП.Видимость = НЕ Объект.БезналичныйРасчет И ДанныеУчетнойПолитики.ПлательщикНСП И Объект.ПоступлениеОтНерезидента;
		Элементы.СуммаНДС.Видимость = Истина;
		Элементы.СуммаНСП.Видимость = НЕ Объект.БезналичныйРасчет И ДанныеУчетнойПолитики.ПлательщикНСП И Объект.ПоступлениеОтНерезидента;
		//Элементы.ЗачетНДС.Видимость = Ложь;
		
		Элементы.ТоварыСуммаРасходовНДС.Видимость 	= Истина;
		Элементы.ТоварыСуммаРасходовНСП.Видимость 	= НЕ Объект.БезналичныйРасчет И ДанныеУчетнойПолитики.ПлательщикНСП И Объект.ПоступлениеОтНерезидента;
		Элементы.ОССуммаРасходовНДС.Видимость 		= Истина;
		Элементы.ОССуммаРасходовНСП.Видимость 		= НЕ Объект.БезналичныйРасчет И ДанныеУчетнойПолитики.ПлательщикНСП И Объект.ПоступлениеОтНерезидента;
...
Процедура ЗаполнитьПоДокументамОснованиям(МассивДокументов)
	...
	Запрос.Текст = 
		...
		|	ОтчетПроизводстваЗаСменуПродукция.СчетУчета,
		|	НЕОПРЕДЕЛЕНО
		|ИЗ
		|	Документ.ОтчетПроизводстваЗаСмену.Продукция КАК ОтчетПроизводстваЗаСменуПродукция
		|ГДЕ
		|	ОтчетПроизводстваЗаСменуПродукция.Ссылка В(&МассивДокументов)
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
...
Процедура РассчитатьСуммыНалогов()
	...
	Если ПризнакСтраны = ПредопределенноеЗначение("Перечисление.ПризнакиСтраны.ЕАЭС")
		И НЕ Объект.ПоступлениеОтНерезидента Тогда
...
//
&НаСервере
Процедура ЗаполнитьПоДокументамОснованиямОС(МассивДокументов)
...
//
&НаСервере
Процедура ДобавитьСтрокиТабличнойЧастиИзДокументаПоступленияОС(МассивДокументов)
