...
Процедура ИнициализироватьДанные(Отказ, РежимПроведения)
	...
	Документы.ПоступлениеАрендованныхОС.ИнициализироватьДанныеДокумента(Ссылка, ДополнительныеСвойства, Отказ);
...
Процедура ОтразитьДвижения(Отказ, РежимПроведения)
	...
	БухгалтерскийУчетСервер.ОтразитьПараметрыУчетаОСВАренде(ДополнительныеСвойства, Движения, Отказ);
...
Процедура ВыполнитьПредварительныйКонтроль(Отказ)
	...
	Запрос.Текст = 
		...
		|ГДЕ
		|	СостоянияОСВАрендеСрезПоследних.Состояние = ЗНАЧЕНИЕ(Перечисление.ВидыСостоянийОСВАренде.Поступило)
		|
		|УПОРЯДОЧИТЬ ПО
		|	НомерСтроки";
