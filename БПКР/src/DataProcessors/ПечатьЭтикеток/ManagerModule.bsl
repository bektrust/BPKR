...
Функция НовыйШтрихкодыМаркируемойПродукции() Экспорт
	...
	ШтрихкодыМаркируемойПродукции.Колонки.Добавить("ТипШтрихкода",        Новый ОписаниеТипов("Строка"));
...
Функция ТабличныйДокументЭтикетка(МассивОбъектов, ШаблонЭтикетки, ШтрихкодыМаркируемойПродукции, КаждаяЭтикеткаНаНовомЛисте) Экспорт
	...
	ПараметрыПечати.Вставить("ТипЦен",                        Справочники.ТипыЦенНоменклатуры.ТипЦенРозничная());
...
Функция ДанныеДляПечатиЭтикеток(СписокНоменклатуры, АдресХранилищаСКД, Знач XMLОписаниеМакета, ТипЦен, ШтрихкодыМаркировки = Неопределено, ШтрихкодыСтрокой = Истина) Экспорт
	...
	Если НЕ ОборудованиеПринтерыЭтикетокВызовСервера.ПолучитьОписаниеМакета(XMLОписаниеМакета, СообщениеОбОшибке, СодержимоеXMLФайла) Тогда
		РезультатВыполнения.Вставить("Успешно", Ложь);
			...
			Если ПолеМакета.Тип = "Barcode" Тогда
				...
				Если ПолеМакета.Значение = "Штрихкод" 
					...
					ПараметрыШтрихкода.Вставить("Штрихкод", 
						РегистрыСведений.ШтрихкодыНоменклатуры.СформироватьШтрихкодEAN13());
...
Функция СведенияОбОрганизациях(ТаблицаОрганизации)
	...
	ОбщегоНазначенияКлиентСервер.ДополнитьТаблицу(ТаблицаОрганизации, ТаблицаСведений);
	...
	Для каждого СтрокаТаблицы Из ТаблицаСведений Цикл
		...
		ЗаполнитьЗначенияСвойств(СтрокаТаблицы, СтрокаТаблицы.Организация);
	КонецЦикла;
...
Функция НоменклатураДляПечатиЦенников(СписокОбъектов, КоличествоПоУмолчанию = 1) Экспорт
	...
	Для каждого СписокОбъектовПоТипу Из ОбщегоНазначенияБПСервер.РазложитьСписокПоТипамОбъектов(СписокОбъектов) Цикл
