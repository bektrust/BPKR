...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ДатаОстатков =  ?(ЗначениеЗаполнено(Объект.Дата), Объект.Дата, ТекущаяДатаСеанса());
	Заголовок = СтрШаблон(НСтр("ru = 'Подбор товаров на дату %1 по складу %2'"), ДатаОстатков, Объект.Склад);
...
Процедура УстановитьПараметрыДинамическихСписков()
	...
	// Счета учета товаров
	МассивСчетовУчетаТоваров = БухгалтерскийУчетСервер.СчетаУчетаТоваров();
	...
	// Счета учета МБП
	МассивСчетовУчетаМБП = БухгалтерскийУчетСервер.СчетаУчетаМБП();
...
Процедура ЗаполнитьДанныеОбъекта()
	...
	Если Параметры.ВидДокумента = "Реализация" Тогда 
		Объект.ТипЦен = Параметры.ДоговорКонтрагента.ТипЦен;
	ИначеЕсли Параметры.ВидДокумента = "РозничнаяПродажа" Тогда
		Объект.ТипЦен = Параметры.Склад.ТипЦенРозничнойТорговли;
	КонецЕсли;
...
//
&НаСервере
Процедура ОбновитьСписокПодобранных(ПриОткрытии = Ложь)
