...
Функция ПредставлениеКассира(Организация) Экспорт
	...
	Иначе
		Кассир = НСтр("ru = 'Администратор (Проверь ТекПользователя)'"); // в чеке обязательно должно быть заполнено представление кассира, иначе
...
// Функция - Параметры розничных продаж
//
// Параметры:
//  Организация	 - СправочникСсылка.Организации	 - получение параметров данной организации
//  Касса		 - СправочникСсылка.Кассы	 - получение параметров данной кассы
// 
// Возвращаемое значение:
//  ПараметрыПродаж - Структура
//
Функция ПараметрыРозничныхПродаж(Организация, Касса = Неопределено) Экспорт
	
	ПараметрыПродаж = Новый Структура;
	ПараметрыПродаж.Вставить("Касса", Справочники.Кассы.ПустаяСсылка());
	ПараметрыПродаж.Вставить("Склад", Справочники.Склады.ПустаяСсылка());
	ПараметрыПродаж.Вставить("Контрагент", Справочники.Контрагенты.ПустаяСсылка());
	ПараметрыПродаж.Вставить("ДоговорКонтрагента", Справочники.ДоговорыКонтрагентов.ПустаяСсылка());
	ПараметрыПродаж.Вставить("СтатьяДвиженияДенежныхСредств", Справочники.СтатьиДвиженияДенежныхСредств.ПустаяСсылка());
	...
	Если НЕ ЗначениеЗаполнено(Касса) Тогда 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПараметрыРозничныхПродаж.Касса = &Касса", "Истина");
	КонецЕсли;	
	...
	Если ВыборкаДетальныеЗаписи.Следующий() Тогда 
    	ЗаполнитьЗначенияСвойств(ПараметрыПродаж, ВыборкаДетальныеЗаписи); 
	КонецЕсли;
	
    Возврат ПараметрыПродаж;
КонецФункции
...
//  ПараметрыПродаж - Структура
//
Функция ПараметрыРозничныхПродажСуществуют(Организация, ИдентификаторУстройства) Экспорт
	
	Если Организация = Неопределено И ИдентификаторУстройства <> Неопределено Тогда
		Организация = ИдентификаторУстройства.Организация	
	КонецЕсли;
	...
	Если НЕ ЗначениеЗаполнено(Организация) Тогда 
		ТекстЗапроса = СтрЗаменить(ТекстЗапроса, "ПараметрыРозничныхПродаж.Организация = &Организация", "Истина");
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапроса;
	Запрос.УстановитьПараметр("Организация", Организация);
...
Функция НовыеДанныеЗакрытияКассовойСмены()
	...
	ДанныеЗакрытия.Вставить("КассирИНН",     ""); // информация о ИНН кассира, которая выводится в кассовые документы
