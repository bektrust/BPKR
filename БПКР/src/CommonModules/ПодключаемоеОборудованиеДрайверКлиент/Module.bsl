...
Процедура НачатьПолучениеОбъектаДрайвера(ОповещениеПриЗавершении, ПараметрыПодключения) Экспорт
	...
	Если ОбъектДрайвера = Неопределено Тогда
		...
		ИначеЕсли ПараметрыПодключения.ПодключениеНеТребуется Тогда
			РезультатВыполнения = Новый Структура();
			РезультатВыполнения.Вставить("Подключено", Истина);
			РезультатВыполнения.Вставить("ПодключаемыйМодуль", "ОнлайнКасса");
...
Процедура НачатьПолучениеОписанияДрайвера_ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ОбъектДрайвера = "ОнлайнКасса" Тогда
	    ДополнительныеПараметры.ОписаниеДрайвера.НаименованиеДрайвера = "СККМ";
		ДополнительныеПараметры.ОписаниеДрайвера.ВерсияДрайвера = "1";
		ДополнительныеПараметры.ОписаниеДрайвера.ПараметрыДрайвера =
			"<?xml version=""1.0"" encoding=""UTF-8""?>
			|<Settings>
			|    <Page Caption=""Параметры"">
			|        <Group Caption=""Общие"">
			|			<Parameter Name=""rnmNumber"" Caption=""Регистрационный номер"" Description=""Регистрационный номер"" TypeValue=""String""/>
			|        	<Parameter Name=""Password"" Caption=""Пароль"" Description=""Пароль пользователя"" TypeValue=""String""/>
			|        	<Parameter Name=""URLService"" Caption=""АдресСервиса"" Description=""Адрес сервиса"" TypeValue=""String""/>
			|        </Group>
			|    </Page>
			|</Settings>";
		РезультатВыполнения = МенеджерОборудованияКлиентСервер.ПараметрыВыполненияОперацииНаОборудовании(Истина);
		РезультатВыполнения.Вставить("ОписаниеДрайвера", ДополнительныеПараметры.ОписаниеДрайвера);
		ДополнительныеДействия = "<?xml version=""1.0"" encoding=""UTF-8""?>
			|<Actions>
			|	<Action Name=""sync"" Caption=""Синхронизировать""/>
			|	<Action Name=""version"" Caption=""О программе""/>
			|</Actions>";
		РезультатВыполнения.Вставить("ДополнительныеДействия", ДополнительныеДействия);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПриЗавершении, РезультатВыполнения);	
	Иначе
...
Процедура НачатьТестУстройства_ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, ДополнительныеПараметры) Экспорт 
	...
	Если ОбъектДрайвера = Неопределено Тогда
		Если ДополнительныеПараметры.ПараметрыПодключения.ИдентификаторОбъекта = "ОнлайнКасса" Тогда
		...
		КонецЕсли;
	ИначеЕсли ОбъектДрайвера = "ОнлайнКасса" Тогда
		РезультатВыполнения = МодульРаботыСОнлайнКассой.ОперацияТест(ДополнительныеПараметры.Параметры);	
		
		ОповещениеПриЗавершении = Новый ОписаниеОповещения("НачатьТестУстройства_Завершение", ЭтотОбъект, ДополнительныеПараметры);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПриЗавершении, РезультатВыполнения);
	Иначе
...
Процедура НачатьВыполнитьДополнительноеДействие_ПолучениеОбъектаДрайвераЗавершение(ОбъектДрайвера, ДополнительныеПараметры) Экспорт 
	...
	ИначеЕсли ОбъектДрайвера = "ОнлайнКасса" Тогда
		...
		Если ИмяДействия = "sync" Тогда 
		    ДанныеОперации = Новый Структура("ИдентификаторУстройства", ДополнительныеПараметры.ПараметрыПодключения.ИдентификаторУстройства);
			РезультатОперации = МодульРаботыСОнлайнКассой.ОперацияАвторизация(ДополнительныеПараметры.Параметры, ДанныеОперации);
		ИначеЕсли ИмяДействия = "version" Тогда	
			РезультатОперации = МодульРаботыСОнлайнКассой.ОперацияВерсия(ДополнительныеПараметры.Параметры);
		КонецЕсли;	
		
		Оповещение = Новый ОписаниеОповещения("НачатьВыполнитьДополнительноеДействие_Завершение", ЭтотОбъект, ДополнительныеПараметры);
		ВыполнитьОбработкуОповещения(ДополнительныеПараметры.ОповещениеПриЗавершении, РезультатОперации); 
	Иначе
		ДополнительныеПараметры.ПараметрыПодключения.Вставить("ОбъектДрайвера", ОбъектДрайвера);
