...
Функция ПодготовитьПараметрыОтчетаНаСервере()
	...
	// Если стоит отбор по валюте и валюта равна валюте регламентированного учета,
	// то необходимо скрыть колонку "Валюта" и строки в валютными суммами.
	// Для этого добавляется параметр "СкрытьВалюты".
	ПараметрыОтчета.СкрытьВалюты = Ложь;
	
	Для Каждого Элемент Из Отчет.КомпоновщикНастроек.Настройки.Отбор.Элементы Цикл
		
		Если Элемент.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Валюта") 
			И Элемент.ПравоеЗначение = Константы.ВалютаРегламентированногоУчета.Получить() Тогда	
			ПараметрыОтчета.СкрытьВалюты = Истина;
			Прервать;
		КонецЕсли;	
	КонецЦикла;
