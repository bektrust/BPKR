...
Процедура Пропустить(Команда)
	...
	Элементы.Список.Обновить();
	Оповестить("Запись_РегулярныеРеализацииТоваровУслуг");
	...
	Если ВыделенныеСтроки.Количество() = 1 Тогда
		...
		ТекстОповещения = НСтр("ru = 'Пропущена реализация:'");
		
		ВалютаДока = ПолучитьВалюту(ДанныеСтроки.Шаблон);
		
		ТекстПояснения = СтрШаблон(НСтр("ru = '%1 %2 %3 перенесен на %4'"),
	...
	Иначе
		...
		ТекстПояснения = СтрШаблон(НСтр("ru = 'Пропущено %1'"),
			...
			ВыделенныеСтроки.Количество(), НСтр("ru = 'реализация, реализации, реализаций'")));
...
Процедура ПрекратитьПовторять(Команда)
	...
	Если ВыделенныеСтроки.Количество() = 1 Тогда
		...
		ТекстОповещения = НСтр("ru = 'Отмена повторения реализации:'");
		
		ВалютаДока = ПолучитьВалюту(ДанныеСтроки.Шаблон);
		
		ТекстПояснения = СтрШаблон(НСтр("ru = '%1 %2 %3'"),
			Строка(ДанныеСтроки.Контрагент),
			Формат(ДанныеСтроки.Сумма, "ЧДЦ=2"),
			ВалютаДока);
	...
	Элементы.Список.Обновить();
	Оповестить("Запись_ПравилаРегулярныхРеализацийТоваровУслуг");
...
Функция СоздатьДокументыНаСервере(Правила)
	...
	Возврат Справочники.ПравилаРегулярныхРеализацийТоваровУслуг.СоздатьДокументыПоПравилам(Правила);
...
Процедура ОткрытьСозданныеДокументы(СозданныеДокументы)
	...
	ОткрытьФорму("Документ.РеализацияТоваровУслуг.ФормаСписка", ПараметрыФормы, ЭтотОбъект, Ложь);
...
Процедура ПропуститьВыбранныеПравилаНаСервере(Правила)
	...
	Справочники.ПравилаРегулярныхРеализацийТоваровУслуг.ПропуститьПовторение(Правила);
...
Процедура ВводДатыСледующего(Результат, ДополнительныеПараметры) Экспорт
	...
	Если Результат < ДатаШаблона Тогда
		...
		ТекстШаблона   = НСтр("ru = 'Следующая реализация не может быть ранее предыдущего от %1'");
...
Процедура ИзменитьДатуСледующегоНаСервере(ДатаСледующего, Правила)
	...
	РегистрыСведений.РегулярныеРеализацииТоваровУслуг.ИзменитьДатуСледующего(Правила, ДатаСледующего);
	
КонецПроцедуры

&НаСервереБезКонтекста
Функция ПолучитьВалюту(Шаблон)
	Возврат НРег(Шаблон.ВалютаДокумента.НаименованиеПолное);
КонецФункции	
