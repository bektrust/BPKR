...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		СтруктураСледующий = Справочники.ПравилаРегулярныхРеализацийТоваровУслуг.Следующий(Объект.Ссылка);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Объект.Наименование = СтрШаблон(НСтр("ru = 'Регулярная реализация: Каждый %1, следующий %2'"),
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Оповестить("Запись_ПравилаРегулярныхРеализацийТоваровУслуг", Шаблон, Объект.Ссылка);
...
Процедура ОбработкаПроверкиЗаполненияНаСервере(Отказ, ПроверяемыеРеквизиты)
	...
	Если ДатаСледующего < ДатаШаблона Тогда
		...
		ТекстШаблона   = НСтр("ru = 'Следующая реализация не может быть ранее предыдущей от %1'");
