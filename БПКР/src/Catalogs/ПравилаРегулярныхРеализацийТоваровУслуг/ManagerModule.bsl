...
// Возвращает правило по реализации
//
// Параметры:
//  РеализацияТоваровУслуг - ДокументСсылка.РеализацияТоваровУслуг
//
// Возвращаемое значение:
//  СправочникСсылка.ПравилаРегулярныхРеализацийТоваровУслуг Или Неопределено, если правило не найдено
//
Функция ПравилоПоРТУ(РеализацияТоваровУслуг) Экспорт
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Шаблон", РеализацияТоваровУслуг);
	Запрос.УстановитьПараметр("Организации", ОбщегоНазначенияБПВызовСервераПовтИсп.ОрганизацииДанныеКоторыхДоступныПользователю());
	
	Запрос.Текст =
	"ВЫБРАТЬ
	|	ПравилаРегулярныхРеализацийТоваровУслуг.Ссылка КАК Правило
	|ИЗ
	|	Справочник.ПравилаРегулярныхРеализацийТоваровУслуг КАК ПравилаРегулярныхРеализацийТоваровУслуг
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.РегулярныеРеализацииТоваровУслуг КАК РегулярныеРеализацииТоваровУслуг
	|		ПО (РегулярныеРеализацииТоваровУслуг.Правило = ПравилаРегулярныхРеализацийТоваровУслуг.Ссылка)
	|ГДЕ
	|	ПравилаРегулярныхРеализацийТоваровУслуг.Организация В(&Организации)
	|	И РегулярныеРеализацииТоваровУслуг.Шаблон = &Шаблон";
...
//
// Параметры:
//   Правило - СправочникСсылка.ПравилаРегулярныхРеализацийТоваровУслуг
//
// Возвращаемое значение:
//   ДатаСледующего
//
...
Функция Следующий(Правило) Экспорт
	...
	СтруктураСледующий.Вставить("Шаблон",         Документы.РеализацияТоваровУслуг.ПустаяСсылка());
	
	Запрос = Новый Запрос;
	Запрос.УстановитьПараметр("Правило", Правило);
	Запрос.Текст =
	"ВЫБРАТЬ
	|	РегулярныеРеализацииТоваровУслугСрезПоследних.Организация,
	|	РегулярныеРеализацииТоваровУслугСрезПоследних.Период КАК ДатаСледующего,
	|	РегулярныеРеализацииТоваровУслугСрезПоследних.Шаблон,
	|	РеализацияТоваровУслуг.Дата КАК ДатаШаблона,
	|	РеализацияТоваровУслуг.Контрагент,
	|	РеализацияТоваровУслуг.СуммаДокумента КАК Сумма
	|ИЗ
	|	РегистрСведений.РегулярныеРеализацииТоваровУслуг.СрезПоследних(, Правило = &Правило) КАК РегулярныеРеализацииТоваровУслугСрезПоследних
	|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	|		ПО РегулярныеРеализацииТоваровУслугСрезПоследних.Шаблон = РеализацияТоваровУслуг.Ссылка
	|			И РегулярныеРеализацииТоваровУслугСрезПоследних.Организация = РеализацияТоваровУслуг.Организация";
...
Функция ДанныеДляНадписиЗапланировано() Экспорт
	...
	|	СУММА(ВЫБОР
	|			КОГДА РегулярныеРеализацииТоваровУслугСрезПоследних.Период < &ТекущаяДата
	|				ТОГДА 1
	|			ИНАЧЕ 0
	|		КОНЕЦ) КАК Просрочено,
	|	МИНИМУМ(РегулярныеРеализацииТоваровУслугСрезПоследних.Период) КАК ДатаСледующего
	|ИЗ
	|	РегистрСведений.РегулярныеРеализацииТоваровУслуг.СрезПоследних(, Правило.Выполняется) КАК РегулярныеРеализацииТоваровУслугСрезПоследних
	|ГДЕ
	|	РегулярныеРеализацииТоваровУслугСрезПоследних.Организация В(&Организации)";
...
Процедура ПропуститьПовторение(Правила) Экспорт
	...
	Для Каждого Правило Из Правила Цикл
		...
		РегистрыСведений.РегулярныеРеализацииТоваровУслуг.УстановитьШаблон(Правило, СтруктураСледующий.Шаблон);
...
//
// Возвращаемое значение:
//   СозданныеДокументы - Массив - Массив ссылок на реализации товаров и услуг.
//
Функция СоздатьДокументыПоПравилам(Правила) Экспорт
	...
	Запрос.Текст =
		"ВЫБРАТЬ
		|	РегулярныеРеализацииТоваровУслугСрезПоследних.Шаблон,
		|	РегулярныеРеализацииТоваровУслугСрезПоследних.Правило
		|ИЗ
		|	РегистрСведений.РегулярныеРеализацииТоваровУслуг.СрезПоследних(, Правило В (&Правила)) КАК РегулярныеРеализацииТоваровУслугСрезПоследних";
	Выборка = Запрос.Выполнить().Выбрать();
	...
	Пока Выборка.Следующий() Цикл
		...
		РегистрыСведений.РегулярныеРеализацииТоваровУслуг.УстановитьШаблон(Выборка.Правило, НовыйДокумент.Ссылка);
