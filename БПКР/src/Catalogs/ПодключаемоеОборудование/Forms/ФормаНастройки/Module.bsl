...
Процедура ДополнительноеДействиеЗавершение(РезультатВыполнения, Параметры) Экспорт
	...
	Если ДрайверОборудования = ПредопределенноеЗначение("Справочник.ДрайверыОборудования.ДрайверКатоОнлайнКасса") Тогда 
		ДополнительноеОписание = РезультатВыполнения.РезультатВыполнения;
		Если ЗначениеЗаполнено(ДополнительноеОписание) Тогда 
			ОбщегоНазначенияКлиент.СообщитьПользователю(ДополнительноеОписание);
		КонецЕсли;	
	КонецЕсли;	
...
Процедура ОбновитьИнтерфейсФормы(ПервыйЗапуск, ПараметрыОперации)
	...
	Если ПараметрыОперации.Результат Тогда
		...
		Если ПараметрыОперации.Свойство("ДополнительныеДействия") Тогда
			ДополнительныеДействия = ПараметрыОперации.ДополнительныеДействия;
		КонецЕсли;
