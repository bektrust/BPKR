...
Асинх Процедура ВопросПодобратьИзСервиса()
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ПодобратьИзКлассификатора();
	Иначе
		ОткрытьФорму("Справочник.КодыПоставокНДС.Форма.ФормаЭлемента",, ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодобратьИзКлассификатора()
	...
	СоответствиеПолей.Вставить("DeliveryType", "ВидПоставки");
	
	ОбщегоНазначенияБПКлиент.ОткрытьФормуПодбораИзКлассификатора(
		"КодыПоставокНДС", 
		"VATDeliveryCodesGetService",     
		НСтр("ru = 'Коды поставок НДС'"),                       
		ЭтаФорма,
		СоответствиеПолей);
КонецПроцедуры
