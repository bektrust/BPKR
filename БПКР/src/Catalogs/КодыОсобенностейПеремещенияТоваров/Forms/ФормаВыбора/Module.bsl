...
Асинх Процедура ВопросПодобратьИзСервиса()
	...
	Если Ответ = КодВозвратаДиалога.Да Тогда
		ПодобратьИзКлассификатора();
	Иначе
		ОткрытьФорму("Справочник.КодыОсобенностейПеремещенияТоваров.Форма.ФормаЭлемента",, ЭтотОбъект);
	КонецЕсли;
КонецПроцедуры
...
Процедура ПодобратьИзКлассификатора()
	...
	ОбщегоНазначенияБПКлиент.ОткрытьФормуПодбораИзКлассификатора(
		"КодыОсобенностейПеремещенияТоваров", 
		"classificationFeaturesMovementGoodsCodesGetService", 
		НСтр("ru = 'Коды особенностей перемещения товаров'"), 
		ЭтаФорма, 
		СоответствиеПолей);
КонецПроцедуры
