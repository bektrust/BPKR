...
Процедура ВставитьИННКППОрганизации(Команда)
	ВставитьПараметр("ИНН_Организации");
КонецПроцедуры

&НаКлиенте
Процедура ВставитьОКПООрганизации(Команда)
	ВставитьПараметр("ОКПООрганизации");
КонецПроцедуры
...
Процедура ВставитьИННКонтрагента(Команда)
	ВставитьПараметр("ИНН_Контрагента");
КонецПроцедуры

&НаКлиенте
Процедура ВставитьОКПОКонтрагента(Команда)
	ВставитьПараметр("ОКПОКонтрагента");
КонецПроцедуры
...
Процедура ЗагрузитьСведенияОДоговореКонтрагента()
	...
	РеквизитыДоговора = ОбщегоНазначения.ЗначенияРеквизитовОбъекта(ДоговорКонтрагента, 
		"Владелец, ФайлДоговора, НомерДоговора, ДатаДоговора");
		
	Контрагент		= РеквизитыДоговора.Владелец;
	ФайлДоговора 	= РеквизитыДоговора.ФайлДоговора;
	НомерДоговора 	= РеквизитыДоговора.НомерДоговора;
	ДатаДоговора 	= РеквизитыДоговора.ДатаДоговора;
	// Считаем, что договор подписан, только если создан файл договора
	ДоговорПодписан = ЗначениеЗаполнено(ФайлДоговора);
...
Функция СписокВложений(ПараметрыОтправки)
	...
	Для каждого ФорматСохранения Из ПараметрыОтправки.ФорматыСохранения Цикл
		...
		Если ПараметрыОтправки.ОбъектыОтправки.Найти("ПриложениеКДоговору") <> Неопределено Тогда
			...
			ТаблицаСведенийСчетНаОплату = Новый ТаблицаЗначений;
			// Конец БПКР
...
Функция ПечатьПриложенияКДоговору(СведенияСчетаНаОплату, ОбъектыПечати, ПараметрыПечати) Экспорт
	...
	Для Каждого СведенияОДокументе Из СведенияСчетаНаОплату Цикл
		...
		СведенияОПоставщике = БухгалтерскийУчетСервер.ПолучитьСведенияОКонтрагенте(СведенияОДокументе.Поставщик);
		ПараметрыОбласти.Вставить("ПредставлениеПоставщика", СведенияОПоставщике.НаименованиеДляПечатныхФорм);
		
		СведенияОПокупателе = БухгалтерскийУчетСервер.ПолучитьСведенияОКонтрагенте(СведенияОДокументе.Покупатель);
		...
		//КонецЕсли;
		ТабличныйДокумент.Присоединить(ОбластьДанных);
		ОбластьСуммы.Параметры.Заполнить(Новый Структура("Всего", БухгалтерскийУчетСервер.ФорматСумм(Сумма)));
		ТабличныйДокумент.Присоединить(ОбластьСуммы);
		...
		Если СуммаНДС <> 0 Тогда
			...
			ОбластьСуммы.Параметры.Заполнить(Новый Структура("ВсегоНДС", БухгалтерскийУчетСервер.ФорматСумм(СуммаНДС)));
		Иначе
		...
		ОбластьСуммы.Параметры.Заполнить(Новый Структура("ИтогоКОплате", БухгалтерскийУчетСервер.ФорматСумм(СуммаКОплате)));
		ТабличныйДокумент.Присоединить(ОбластьСуммы);
		...
		ИтоговаяСтрока = СтроковыеФункцииКлиентСервер.ПодставитьПараметрыВСтроку(НСтр("ru='Всего наименований %1, на сумму %2'"),
			НомерСтроки, БухгалтерскийУчетСервер.ФорматСумм(СуммаКПрописи, СведенияОДокументе.Валюта));
		СуммаПрописью = БухгалтерскийУчетСервер.СформироватьСуммуПрописью(СуммаКПрописи, СведенияОДокументе.Валюта);
...
&НаСервереБезКонтекста
Процедура РаспределитьСуммуПоКолонкеТаблицы(РаспределяемаяСумма, Таблица, ИмяКолонки, ИмяКолонкиБазы = "")
